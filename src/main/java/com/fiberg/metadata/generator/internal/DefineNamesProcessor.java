...
import com.fiberg.metadata.generator.Metaname;
...
@SupportedAnnotationTypes("com.fiberg.metadata.generator.Metadata")
public class DefineNamesProcessor extends AbstractProcessor {
	...
	private Map<TypeElement, DefineNameClassWriter> createClassWriter(
		...
		for (final Element element : env
				.getElementsAnnotatedWith(Metadata.class)) {
			final ElementKind kind = element.getKind();
			if (kind.isClass()) {
				final TypeElement typeElement = (TypeElement) element;
				logNote("Create defname of class \"" + typeElement.getSimpleName() + "\".");
				final boolean withStaticField = typeElement.getAnnotation(Metadata.class).withStaticField();
				...
				for (final Element member : members) {
					...
					final Metaname constant =
							member.getAnnotation(Metaname.class);
					if (constant != null) {
	...
	private DefineNameClassWriter getOrCreateClassWriter(
		...
		if (targetClass == null) {
			...
			final String classSimpleName = typeElement.getSimpleName()
					.toString();
			final Metadata.Case[] cases = typeElement.getAnnotation(
